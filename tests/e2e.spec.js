...
test.describe('OPC 端到端测试套件', () => {
  ...
  test('1. 落地页加载 + Ma 間 Hero 元素', async ({ page }) => {
    ...
    await expect(page).toHaveTitle(/OPC 节点百科/);
    // 验证页面有主要 CTA 入口
    const startBtn = page.getByRole('link', { name: /开始|测试|开始测试/ }).first();
  ...
  test('2. 知识图谱渲染 + 节点数量验证', async ({ page }) => {
    ...
    await page.waitForFunction(() => {
      ...
      return circles.length >= 50; // 容差
    }, { timeout: 15000 });
    const circleCount = await page.locator('svg#graph circle').count();
    expect(circleCount).toBeGreaterThanOrEqual(50);
  });
  ...
  test('3. 节点页面加载 + Ma 間 排版', async ({ page }) => {
    ...
    // 验证 h1 字号在 36-48px 之间（Ma 間 排版规范）
  ...
  test('5. OPC 适配测试加载 + questions.json 完整', async ({ page }) => {
    ...
    await expect(page).toHaveTitle(/OPC 适配测试/);
  ...
  test('6. CTA 归因跳转 + attribution.json 记录', async ({ page, request }) => {
    ...
    const response = await request.get(`${BASE_URL}/go/wechat?from=node-31`, {
      maxRedirects: 0,
